...
class SettingsHomePage(BasePage):
    ...
    SETTINGS_APP = (
        BasePage.By.IOS_PREDICATE,
        "type == 'XCUIElementTypeApplication' AND (name == 'Settings' OR label == 'Settings')",
    )
    INITIAL_SCREEN_ANCHOR_NAMES = {
        "com.apple.settings.airplaneMode",
        "com.apple.settings.wifi",
    ...
    def get_home_readiness_failures(self) -> list[str]:
        """
        Return visual readiness failures for the Settings initial screen.
    ...
    def assert_home_visually_ready(self) -> None:
        """Wait within one timeout budget for a usable Settings home."""
        failures: list[str] = []

        def ready(_driver: WebDriver) -> bool:
            nonlocal failures
            failures = self.get_home_readiness_failures()
            return not failures

        try:
            WebDriverWait(self.driver, self.DEFAULT_TIMEOUT).until(ready)
        except TimeoutException as error:
            raise AssertionError(
                "Settings app not visually ready: " + "; ".join(failures)
            ) from error
